...
|---|---|---|---|
| **Hugging Face Spaces** (Docker SDK) | да, CPU Basic, бессрочно | **16 GB** | ✅ выбран |
...
**HF Spaces выбран**, потому что это единственный из трёх, у кого бесплатный
тариф реально тянет headless Chromium:

- 2 vCPU / 16 GB RAM (CPU Basic, бесплатно);
- Docker-образы поддерживаются нативно (`sdk: docker` в front matter README);
- HTTPS «из коробки» на `https://<user>-<space>.hf.space`;
- нет платы за трафик в разумных пределах; outbound HTTPS (80/443/8080)
  разрешён — этого достаточно для обращений к OpenRouter/OpenAI;
- **ограничения**: Space «засыпает» после ~48 ч неактивности → холодный старт
  от десятков секунд до ~1 минуты на первый запрос (демо-страница явно
  предупреждает об этом); нет персистентного диска на free-тарифе (демо в
  нём и не нуждается — всё эфемерно); сеть наружу только на портах
  80/443/8080 (наш провайдер-трафик — HTTPS 443, ок).
...
2. **Ключ никогда не логируется.** Функция редактирования
   ...
   в логах есть `[REDACTED]` и нет сырого ключа.
...
- [x] `pytest tests/demo/` — 19 тестов: демо выключено по умолчанию, allowlist,
      rate-limit 429, таймаут, маскирование ключа в логах/ответах (включая
      трейсбек), HTTPS-граница, эфемерность артефактов.
- [x] `ruff check .` и `mypy src` — чисто.
- [x] `docker build` + smoke: `/livez`, `/demo`, `/demo/tasks` отвечают.
- [ ] Ручной прогон с реальным ключом на публичном URL (после деплоя по §4).
