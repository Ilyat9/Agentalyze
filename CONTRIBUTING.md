...
* **Анимированные CLI-демки** (`docs/assets/cli-*.gif`) — сгенерированные
  артефакты из VHS-тapes в [`docs/tapes/`](docs/tapes/). Если ваш PR меняет
  видимый вывод CLI (список задач, формат summary), перегенерируйте их:
  `brew install vhs && scripts/gen_cli_demos.sh`. Демки обязаны показывать
  только реальный вывод; сценарии с моделью реплеят настоящие трейсы.
